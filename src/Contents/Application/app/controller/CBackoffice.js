App.controller.define('CBackoffice', {
	...
	admin_add_click: function()
	{
		...
		App.DB.get('sapei://@'+App.get('TAdmin combo#combo_db').getValue(),function(r) {
			var e={};
	...
	admin_remove_click: function()
	{
		...
		App.DB.get('sapei://@'+App.get('TAdmin combo#combo_db').getValue(),function(r) {
			...
			Ext.MessageBox.confirm('Admin', 'Voulez vous vraiment supprimer cet enregistrement ?', function(btn){
				if (btn === 'yes') App.DB.del('sapei://'+App.get('TAdmin combo#combo_db').getValue(),[sm.selected.items[0].data[key]],function(e,r) {
	...
	selectDB: function(p) {
		var store=App.store.create('sapei://'+p.getValue());
		var grid=App.get('TAdmin grid#dbgrid');
		...
		App.DB.get('sapei://@'+App.get('TAdmin combo#combo_db').getValue(),function(r) {
			...
			for(var i =0; i < records.length; i++){
				...
				if(rec.dirty == true){
					...
					App.DB.post('sapei://'+App.get('TAdmin combo#combo_db').getValue(),rec.data,function(e,r) {
						App.get('TAdmin grid#dbgrid').getStore().load();
					});
				}
			}			
